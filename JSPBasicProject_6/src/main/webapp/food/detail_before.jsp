...
<%--
	Cookie
	HTTP 프로토콜 새로운 페이지가 실행되면 기존의 입력된 모든 정보가 사라진다
	----------- 로그인 => 새로운 페이지로 이동 (로그인 정보는 사라진다)
	----------- 지속성(유지성)이 없다
				---------------- 정보를 유지할 수 있게 만든 라이브러리 => Session/Cookie
			   ...
			   ## response
			   	  => 한개 JSP에서 한가지 일만 수행
			   	  	 -------   --------
			   	  	 			...
			   	  	 			Cookie 전송	=> 1
			   	  	 			=> 여러번 전송이 가능
			2. 읽기 : Cookie[] cookies = request.getCookie();
...
<%
	...
	if(cookies!=null){
		for(int i = 0; i < cookies.length; i++){
			if(cookies[i].getName().equals("house_"+fno)){ // 이지 저장된 쿠키면 삭제후 다시 생성 => 갱신

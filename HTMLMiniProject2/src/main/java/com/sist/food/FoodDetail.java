...
public class FoodDetail extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 FoodVO vo=dao.foodDetailData(Integer.parseInt(fno));
			...
			List<ReplyVO> list=rdao.foodReplyListData(Integer.parseInt(fno));
			...
			for(ReplyVO rvo:list)
			{
				...
				if(rvo.getId().equals(id))
				{
					// <html> => 태그는 사용자 정의가 없다 속성은 사용자 정의가 가능
					 ...
					 out.println("<a href=FoodReplyInsert?fno="+fno+"&rno="+rvo.getRno()+" class=\"btn btn-xs btn-info\">삭제</a>");
				}
				out.println("</td>");	
				out.println("</tr>");
				out.println("<tr>");
				...
				out.println("<form method=post action=FoodReplyUpdate>");
			...
			{
			 out.println("<form method=post action=FoodReplyInsert>");
			 out.println("<table class=table>");
			 out.println("<tr>");
			 out.println("<td>");

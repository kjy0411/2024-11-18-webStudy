...
// 상세보기 
	public FoodVO foodDetailData(int fno) {
		...
		try {
			...
			// mainpage => 공동작업
			/*
			 * private int fno,hit; private double score; private String
			 * name,type,phone,address,theme,poster, images,time,parking,content,price;
			 */
...
// 음식 종류별 검색 
	public List<FoodVO> foodTypeFind(int page, String type) {
		...
		try {
			 ...
			 * +"FROM (SELECT fno,name,poster " +"FROM music " +"WHERE cno=?)) "
			 * +"WHERE num BETWEEN ? AND ?";
			 */
			if (!type.equals("기타")) {
